    ...
    class SimpleDB
      class Real
        ...
        def encode_attribute_names(attributes)
          ...
          #original:                                                    error here v
          #AWS.indexed_param('AttributeName', attributes.map {|attribute| attributes.to_s})
          #
          # what the original outputs:
          #   {"AttributeName.1"=>"[:state, :deployment]", "AttributeName.2"=>"[:state, :deployment]"}
          #
          # what the fixed version outputs:
          #   {"AttributeName.1"=>"state", "AttributeName.2"=>"deployment"}
        end
      end
    end
  end
...
class FogSimpleDBWrapper
  def initialize(collection, node_name, ec2_region=nil)
    if not FOGFOUND
      raise RequirementError, "Aborting: The fog library is missing!"
    end
    ...
    fog = Fog::AWS::SimpleDB.new(
      Chef::Config[:use_iam_role] ? {use_iam_profile: true} : _get_boto_creds() )
  ...
  def _put(item, attributes, options = {})
    ...
    attributes.each_pair do |key, val|
      if val.nil?
        ...
        to_store[key] = val unless val.empty?
      else
...
class ProxyBase < BasicObject
  def instance_of?(kls)
    ::Kernel.instance_method(:instance_of?).bind(self).call(kls)
  end
  def object_id
...
class PersistList < ProxyBase
  MODIFIES = [:<<, :[]=, :clear, :collect!, :map!, :compact!, :concat, :delete, :delete_at, :delete_if, :fill, :flatten!, :insert, :pop, :push, :reject!, :replace, :shift, :slice!, :uniq!, :unshift]
  ...
  def method_missing(method, *args, &block)
    return super(method, *args, &block) unless @_wrapped.respond_to? method
    if MODIFIES.include? method.to_sym
  ...
  def respond_to?(method, include_private = false)
    @_wrapped.respond_to?(method, include_private) or super
  end
end

class FogRetryProxy < ProxyBase
  MAX_HTTP_RETRY = 20
  def initialize(wrap)
    @_wrapped = wrap
  end
  def method_missing(method, *args, &block)
    ...
    Kernel.loop do
      ...
      Kernel.sleep (1.0+0.2*attempts.to_f)
      attempts += 1
    end
  end
  def respond_to?(method, include_private = false)
...
def persistent_mash_factory_function(fqdn, env)
  PersistentMash.new(
    PersistWrapper.new(
      FogSimpleDBWrapper.new(
        Chef::Config[:sdb_domain],
        fqdn),
        env))
end
